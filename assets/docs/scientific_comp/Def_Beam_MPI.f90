Program Def_Beam_MPI
! Corey Zhang
! This program parallelize the code for Jacobi Method
! Solve the deflection of the beam equation.

use mpi

integer, parameter :: N = 119
real :: h, alpha, beta, x, lb, ub, c2
real, dimension(N) :: a, b, c, d
real, dimension(0:N+1) :: Ja
real :: p, q, r
...
if (rank == root) then
   ! Solve Ax = b by calling Jacobi method
   call jacobi(a, b, c, d, N, alpha, beta, lb, h, Ja)
   print *, "   x       jacob method"
   do i = 0, N + 1
      print '(F6.1,5x,F9.6,5x,F9.6)', lb + i * h, Ja(i)
   end do   
end if
